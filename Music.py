...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    @classmethod
    async def search1_source(cls, ctx: commands.Context, search: str, *, loop: asyncio.BaseEventLoop = None):
        channel = ctx.channel
        loop = loop or asyncio.get_event_loop()

        cls.search1_query = '%s%s:%s' % ('ytsearch', 10, ''.join(search))
...
class Music(commands.Cog):
    ...
    async def _summon(self, ctx: commands.Context, *, channel: discord.VoiceChannel = None):
        ...
        em.add_field(name = '<a:Hy:781962182220840990> Summons the bot to a voice channel \nIf no channel was specified, it joins your channel'  , value = "made by PP_ Swag and $wip")
        await ctx.send(embed = em)
        

        if not channel and not ctx.author.voice:
            raise VoiceError(em = discord.Embed(color = color).em.add_field(name = "You are neither connected to a voice channel nor specified a channel to join !") , value = "made by PP_ Swag and $wip")
    ...
    async def _leave(self, ctx: commands.Context):
        ...
        if not ctx.voice_state.voice:
            return await ctx.send(em = discord.Embed(color = color).em.add_field(name = "<a:Hy:781962182220840990> Not connected to any voice channel.!") , value = "made by PP_ Swag and $wip")
    ...
    async def _volume(self, ctx: commands.Context, *, volume: int):
        ...
        embed.add_field(name = 'Sets the volume of the player' , value = "made by PP_ Swag and $wip")
        ...
        if volume < 0:
            embed = discord.Embed(color = color)
            embed.add_field(name = "<a:Hy:781962182220840990> Volume must be between 0 and 100 !" , value = "made by PP_ Swag and $wip")
        ...
        embed = discord.Embed(color = color)
        embed.add_field(name = f"<a:Hy:781962182220840990> Volume of the player set to %.{format(volume)}" , value = "made by PP_ Swag and $wip")
        await ctx.send(embed = embed)
...
client = commands.Bot(command_prefix='d!', case_insensitive=True, description="The Superior Bot")
