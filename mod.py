...
from helpers import create_mute_role
...
class Moderator(commands.Cog):
    ...
    async def mute(self, ctx, user: discord.Member = None, time: str = None):
        if user is None:
            em = discord.Embed(color = THEME_COLOR)
            em.add_field(name = "Insufficient arguments" , value = "made by $wip")
            await ctx.send(embed = em)
        elif ctx.author.top_role.position <= user.top_role.position and ctx.guild.owner.id != ctx.author.id:
            em = discord.Embed(color = THEME_COLOR)
            em.add_field(name = "You cannot mute this user because their role is higher than or equal to yours." , value = "made by $wip")
            await ctx.send(embed = em)
        else:
            guild = ctx.guild
            mute_role = None
            ...
            if mute_role in user.roles:
                em = discord.Embed(color = THEME_COLOR)
                em.add_field(name = "This user is already muted" , value = "made by $wip")
                await ctx.send(embed = em)
                ...
                if not mute_role:
                    em = discord.Embed(color = THEME_COLOR)
                    em.add_field(name = "This server does not have a `Muted` Role. Creating one right now" , value = "made by $wip")
                    await ctx.send(embed = em)
                ...
                if time is None:
                    ...
                    em = discord.Embed(color = THEME_COLOR)
                    em.add_field(name = f"User {user.mention} has been muted! They cannot speak." , value = "made by $wip")
                    await ctx.send(embed = em)
...
#                await ctx.send(f"{username} is has not been banned in this server.")
    ...
    async def lockchannel(self, ctx, channel: discord.TextChannel = None):
        ...
        em = discord.Embed(color = THEME_COLOR)
        em.add_field(name = f"<a:Hy:781962182220840990> 🔒The channel {channel.mention} has been locked" , value = "made by $wip")
        await ctx.send(embed = em)
    ...
    async def unlockchannel(self, ctx, channel: discord.TextChannel = None):
        ...
        em = discord.Embed(color = THEME_COLOR)
        em.add_field(name = f"<a:Hy:781962182220840990> 🔓The channel {channel.mention} has been unlocked" , value = "made by $wip")
        await ctx.send(embed = em)
